...
const BudgetTracking = ({ userId }) => {
    ...
    <div className="budget-tracking">
      {totals && (
        <div className="text-center mb-6">
          <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-100">{t('totalCost')}</h3>
          <p className="text-lg text-blue-500">{totals?.totalOverallCost || 0} CZK</p>
        </div>
      )}

      <div className="flex justify-center items-center mb-6">
        <label htmlFor="chartType" className="mr-3 text-gray-700 dark:text-gray-100 font-medium">
        {t('budgetChart')}
        </label>
        <select
          id="chartType"
  ...
  {chartType !== savedChartType && (
    <button
      ...
      className="ml-3 p-2 text-black dark:text-white focus:outline-none"
  ...
  {chartType === savedChartType && (
    <div className="ml-3 p-2">
      ...
      <div className="bg-gray-50 dark:bg-black p-4 rounded-lg shadow-inner">{renderChart()}</div>
    </div>
  );
};
