...
const TripBudgetTracking = ({ userId, tripId }) => {
    ...
    const fetchDefaultChartType = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/chart-type?userId=${userId}`,
  ...
  useEffect(() => {
    const fetchBudgetData = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/tripBudget?id=${userId}&tripId=${tripId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
...
TripBudgetTracking.propTypes = {
  userId: PropTypes.number.isRequired,
  tripId: PropTypes.number.isRequired,
};

export default TripBudgetTracking;
